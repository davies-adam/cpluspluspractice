...
MessageStatus check_header(const char *email_address, const char *header, const char *filename) {
    int colon_count = 0;
    char parsed_header[100];
    char *sent_email, *sent_digest, *sent_counter;
    strcpy(parsed_header, header);

    sent_email = strtok(parsed_header, ":");
    sent_digest = strtok(nullptr, ":");
    sent_counter = strtok(nullptr, ":");

    if(sent_email == nullptr || sent_digest == nullptr || sent_counter == nullptr) {
